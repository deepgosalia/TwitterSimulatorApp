defmodule Engine do
  ...
  def start_link(arg) do
    ...
    :ets.new(:send_table, [:set, :public, :named_table])
    :ets.new(:recv_table, [:set, :public, :named_table])

    :ets.new(:mysubs, [:set, :public, :named_table])


    :ets.new(:nameToId, [:set, :public, :named_table])
  ...
  def subscribe(usr,sub) do
    ...
    IO.puts("herererererererereradadad")
    if result do
      :true
    else
      :false
  ...
  def handle_call({:addSub,usr,sub},_from,state) do
    ...
    if sub != usr do
      ...
      :ets.insert(:mysubs,{usr, Enum.uniq(mysub)})
    end
   ...
   # IO.inspect(:ets.lookup(:msgTable, msg_id))
    [{_,[message,_from]}] = :ets.lookup(:msgTable, msg_id)
      ...
      if(:ets.member(:mentions_table,x)) do
        ...
        :ets.insert(:mentions_table,{x,[msg_id]})
      end
      end)
  ...
  def parseMsg(msg_id,uid) do
    [{_,[message,_from,_type]}] = :ets.lookup(:msgTable, msg_id)
      charlist = String.split(message," ")
      hashtags = Enum.filter(charlist, fn x-> String.starts_with?(x,"#")==true end)
      users = Enum.filter(charlist, fn x-> String.starts_with?(x,"@")==true end)

      Enum.each(users,fn x ->
      x = String.slice(x, 1..String.length(x)-1)

      sub=:crypto.hash(:sha, x) |> Base.encode16 |> String.slice(0..5) |> String.to_integer(16)
      [{_,msg}] = :ets.lookup(:send_table,sub)
      msg1=[msg_id | msg]
      :ets.insert(:send_table,{sub,msg1})

      if(:ets.member(:mentions_table,x)) do  # x == deep
        [{_,msg}] = :ets.lookup(:mentions_table,x)
        msg1=msg++[msg_id]
        :ets.insert(:mentions_table,{x,msg1})
      else
