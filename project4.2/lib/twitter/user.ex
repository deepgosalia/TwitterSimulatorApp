defmodule User do
  ...
  def start_link(usr_info) do
    ...
    :ets.insert(:mysubs, {usr_id,[]})
    :ets.insert(:mentions_table, {usr_id,[]})
    :ets.insert(:send_table, {usr_id,[]})
    :ets.insert(:recv_table, {usr_id,[]})
  ...
  def send_message(uid, message) do
    ...
    m_count = Enum.random(1000..3000)
    id = Integer.digits(uid) ++ Integer.digits(m_count)
    id = Integer.to_string(Integer.undigits(id))

    #IO.puts("#{message} is #{id}")
    type = 0
    :ets.insert(:msgTable,{id,[message,uid,type]})

    [{_,msg}] = :ets.lookup(:send_table,uid)
    msg1=[id | msg]
    :ets.insert(:send_table,{uid,msg1})

    Engine.parseMsg(id,uid)
    Engine.distMsgs(id,uid)
    #add to message list
